...
class OtpController extends Controller
{
    ...
    public function verifyOTP(Request $request){
        ...
        if($request->type == "email") {
            $match =  ['otp'=>$request->otp,'status'=>0];
            ...
            try {
                // if($request->otp == "121212") {
                //     return response()->json(['success' => true, 'status' => 200, 'data' => null]);
                // }
                $to = '+91' . $request->mobile;
                $result = $this->twilio->checkVerification($to, $request->otp);

                if($result->status == 'approved') {
                    return response()->json(['success' => true, 'status' => 200, 'data' => null]);
                }
    
                return response()->json(['success' => false, 'status' => 404, 'data' => null]);
            } catch(Exception $e) {
                Log::error($e->getMessage());
                return response()->json(['success' => false, 'status' => 500, 'data' => null]);
            }
        }
