...
class ProductsController extends Controller
{
    ...
    public function getById(Request $request){
        ...
        $salon = Salon::where('uid', $data->freelacer_id)
        ->selectRaw('uid as id, name as first_name, " " as last_name, status')
        ->first();
        $storeInfo = $salon ?: User::select('id','first_name','last_name','status')
        ->where('id', $data->freelacer_id)
        ->first();
    ...
    public function getProducts(Request $request){
        ...
        $activeUserIds = User::whereIn('type', ['individual', 'freelancer', 'salon'])->where('status', '1')->pluck('id');
        $data = Products::where(['cate_id'=>$request->cate_id,'sub_cate_id'=>$request->sub_cate_id])->whereIn('freelacer_id', $activeUserIds)->get();
