...
class AuthController extends Controller
{
    ...
    public function login(Request $request){
        ...
        if($request->type == 'partner') {
            if(!in_array($user->type, ['salon', 'individual', 'freelancer'])) {
                return response()->json(['error' => 'access denied'], 401);
            }
        }
        ...
        if($user->type == 'individual' || $user->type == 'freelancer'){
    ...
    public function uploadImage(Request $request){
        ...
        $remoteDir = "uploads/{$uploadFolder}";
        $remotePath = "{$remoteDir}/{$imageName}";
        try {
            // Upload to DigitalOcean Spaces using a stream-backed put to avoid undefined putFileAs on the contract
            Storage::disk('spaces')->put(
                $remotePath,
                fopen($image->getRealPath(), 'r'),
                [
                    'visibility' => 'public', // change to 'private' if you want private objects
                    // 'CacheControl' => 'max-age=31536000, public', // optional
                ]
            );
        } catch (\Throwable $e) {
            Log::error('Spaces upload failed', ['err' => $e->getMessage()]);
    ...
    public function sendOTP(string $to, TwilioService $twilio) {
        $otp = random_int(100000, 999999);
        $twilio->sendSMS($to, 'Dear Customer,\nYour Verification code is: ' . $otp . '\n-Team Papabear');
    }
        ...
        if (is_null($data) || !$data) {
            ...
            if($settings->sms_name =='0'){ // send with twillo
                ...
                try{
                    // $otp = 121212;
                    // $otp = random_int(100000, 999999);
                    $this->twilio->sendVerification($to);
                    // $client = new \GuzzleHttp\Client();
                    // $response = $client->request('POST', $url, [
                    //     'headers' =>
                    //     [
    ...
    public function deleteAccount(Request $request) {
        $validator = Validator::make($request->all(), [
            'uid' => 'required|unique:account_deletions,user_id',
        ], [
            'uid.unique' => 'Your deletion request is already in process.',
        ]);
        if ($validator->fails()) {
            $response = [
                'success' => false,
        ...
        AccountDeletion::create([
            'user_id'=>$request->uid,
            'deletion_time' => Carbon::now()->setTimezone('Asia/Kolkata')->format('d-m-Y H:i:s'),
        ]);
        $response = [
            'data'=>true,
            'success' => true,
