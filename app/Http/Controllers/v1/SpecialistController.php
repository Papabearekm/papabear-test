...
class SpecialistController extends Controller
{
    public function save(Request $request){
        ...
        $data = Specialist::create($request->all());
        if (is_null($data)) {
            $response = [
            'data'=>$data,
            'message' => 'error',
    ...
    public function getById(Request $request){
        ...
        $data = Specialist::find($request->id);
    ...
    public function update(Request $request){
        ...
        $data = Specialist::find($request->id)->update($request->all());
    ...
    public function getBySalonID(Request $request){
        ...
        $data = Specialist::where(['salon_uid'=>$request->id])->get();
        foreach($data as $loop){
            $ids = explode(',',$loop->service_id);
            $loop->categories = Services::select('id','name','cover')->WhereIn('id',$ids)->get();
        }
    ...
    public function getSpecialist(Request $request){
        ...
        $data = Specialist::where(['salon_uid'=>$request->id,'status'=>1])->get();
        foreach($data as $loop){
    ...
    public function getActiveSpecialist(Request $request){
        ...
        $data = Specialist::where(['status'=>1,'salon_uid'=>$request->id])->get();
        $response = [
            'data'=>$data,
            'success' => true,
            'status' => 200,
            ...
            if (isset($row[0])) {
                if ($row[0] != "") {
                    ...
                    if(count($header) == count($row)){
                        ...
                        $checkLead  =  Specialist::where("id", "=", $row["id"])->first();
