...
<body>
    ...
    <table class="items-table">
        ...
        <tbody>
            ...
            @endphp
            @foreach ($items['products'] as $serviceItem)
                @php
                    $serviceDetails = DB::table('products')->where('id', $serviceItem['id'])->first();
                    $totalWithoutTax = (float)$serviceItem['sell_price'];
                    $total = $totalWithoutTax * (float)$serviceItem['quantity'];
                @endphp
                <tr>
                    <td>{{ $loop->index + 1 }}</td>
                    ...
                    <td>18</td>
                    <td>₹ {{ number_format(($total * 0.18), 2) }}</td>
                    <td>₹ {{ number_format($total + ($total * 0.18), 2) }}</td>
                </tr>
            @endforeach
            @if ($items['distance_cost'])
                <tr>
                    <td>{{ sizeof($items['products']) + 1 }}</td>
